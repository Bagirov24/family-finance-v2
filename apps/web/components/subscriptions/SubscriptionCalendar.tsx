...
export function SubscriptionCalendar({ subscriptions }: Props) {
  ...
  // Определяем язык по первому доступному переводу
  const isRu = t('per_month') === 'В месяц'
  ...
  const firstWeekday = weekdayMon(startOfMonth(year, month)) // 0-6
  const totalDays = daysInMonth(year, month)
  const todayDay = isSameDay(today, new Date(year, month, today.getDate())) && today.getMonth() === month && today.getFullYear() === year ? today.getDate() : -1

  // Сетка: пустые ячейки + дни
