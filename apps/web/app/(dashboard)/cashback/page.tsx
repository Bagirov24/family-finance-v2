...
export default function CashbackPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <section>
        <h2 className="text-sm font-semibold text-muted-foreground mb-3 uppercase tracking-wide">{t('my_cards')}</h2>
        {isLoading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {cards.map(card => (
              ...
              >
                ...
                <p className="text-sm mt-1 opacity-90">{t('card_type')}: {card.card_type}</p>
                <p className="text-sm mt-2 opacity-90">{t('default_cashback')}: {card.default_cashback_percent}%</p>
              ...
              return (
                <div key={cat.id} className="flex items-center justify-between py-1 gap-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-sm truncate">{tcat(cat.name_key, { defaultValue: cat.name_key })}</span>
                  </div>
                  {best ? (
                    <span className="text-xs font-semibold text-primary shrink-0">{best.cardName} • {best.percent}%</span>
                  ) : (
